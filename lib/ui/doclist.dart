import 'dart:async';

import 'package:docExpiry/model/model.dart';
import 'package:docExpiry/ui/docdetail.dart';
import 'package:docExpiry/util/dbhelper.dart';
import 'package:docExpiry/util/utils.dart';
import 'package:flutter/material.dart';

const menuReset = "Reset Local Data";
List<String> menuOptions = const <String>[menuReset];

class DocList extends StatefulWidget {
  @override
  _DocListState createState() => _DocListState();
}

class _DocListState extends State<DocList> {
  DbHelper dbHelper = DbHelper();
  List<Doc> docs;
  int count = 0;
  DateTime currentDate;

  @override
  void initState() {
    super.initState();
  }

  Future getData() async {
    final dbFuture = dbHelper.initializeDb();
    dbFuture.then(
        // result here is the actual reference to the database object
        (result) {
      final docsFuture = dbHelper.getDocs();
      docsFuture.then(
          // result here is the list of docs in the database
          (result) {
        if (result.length >= 0) {
          List<Doc> docList = List<Doc>();
          var count = result.length;
          for (int i = 0; i <= count - 1; i++) {
            docList.add(Doc.fromObject(result[i]));
          }
          setState(() {
            if (this.docs.length > 0) {
              this.docs.clear();
            }
            this.docs = docList;
            this.count = count;
          });
        }
      });
    });
  }

  // checking for date and time discrepancies
  void _checkDate() {
    const secs = const Duration(seconds: 10);

    new Timer.periodic(secs, (Timer t) {
      DateTime now = DateTime.now();

      if (currentDate.day != now.day ||
          currentDate.month != now.month ||
          currentDate.year != now.year) {
        getData();
        currentDate = DateTime.now();
      }
    });
  }

  // Navigate to the doc details
  void navigateToDetail(Doc doc) async {
    bool result = await Navigator.push(
        context, MaterialPageRoute(builder: (context) => DocDetails(doc)));
    if (result == true) {
      getData();
    }
  }

  void _showResetDialog() {
    showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            title: new Text("Reset"),
            content: new Text("Dou you want to delete all local data?"),
            actions: <Widget>[
              FlatButton(
                onPressed: () {
                  Navigator.of(context).pop();
                },
                child: new Text('Cancel'),
              ),
              FlatButton(
                onPressed: () {
                  Future f = _resetLocalData();
                  f.then((result) => Navigator.of(context).pop());
                },
                child: new Text('OK'),
              ),
            ],
          );
        });
  }

  // Resetting local data
  Future _resetLocalData() async {
    final dbFuture = dbHelper.initializeDb();
    dbFuture.then((result) {
      final dDocs = dbHelper.deleteRows(DbHelper.tblDocs);
      dDocs.then((result) {
        setState(() {
          this.docs.clear();
          this.count = 0;
        });
      });
    });
  }

  // Selecting the menu option
  void _selectmenu(String value) async {
    switch (value) {
      case menuReset:
        _showResetDialog();
    }
  }

  ListView docListItems() {
    return ListView.builder(
        itemCount: count,
        itemBuilder: (BuildContext context, int position) {
          String dd = Validate.getExpiryStr(this.docs[position].expiration);
          String dl = (dd != "1") ? " days left" : "day left";
          return Card(
            color: Colors.white,
            elevation: 1.0,
            child: ListTile(
              leading: CircleAvatar(
                backgroundColor:
                    (Validate.getExpiryStr(this.docs[position].expiration) !=
                            "0")
                        ? Colors.blue
                        : Colors.red,
                child: Text(
                  this.docs[position].id.toString(),
                ),
              ),
              title: Text(this.docs[position].title),
              subtitle: Text(Validate.getExpiryStr(
                      this.docs[position].expiration) +
                  dl +
                  "\nExp: " +
                  DateUtils.convertToDateFull(
                      DateUtils.convertToDate(this.docs[position].expiration))),
              onTap: () {
                navigateToDetail(this.docs[position]);
              },
            ),
          );
        });
  }

  @override
  Widget build(BuildContext context) {
    this.currentDate = DateTime.now();

    if (this.docs == null) {
      this.docs = List<Doc>();
      getData();
    }

    _checkDate();

    return Scaffold(
      resizeToAvoidBottomPadding: false,
      appBar: AppBar(
        title: Text('DocExpiry'),
        actions: <Widget>[
          PopupMenuButton(
            onSelected: _selectmenu,
            itemBuilder: (context) {
              return menuOptions.map((choice) {
                return PopupMenuItem(value: choice, child: Text(choice));
              }).toList();
            },
          )
        ],
      ),
      body: Center(
        child: Scaffold(
          body: Stack(
            children: <Widget>[
              docListItems(),
            ],
          ),
          floatingActionButton: FloatingActionButton(onPressed: (){
            navigateToDetail(Doc.withId(-1, "", "", 1, 1, 1, 1));
          },
          tooltip: "Add new doc",
          child: Icon(Icons.add),),
        ),
      ),
    );
  }
}
